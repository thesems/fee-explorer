...
for processing these events embedded within blocks. It shall provide API functionality to retrieve collected
events for a given integrator and store them in MongoDB using the typegoose framework.
...
- Provide historical lookup of FeeCollector events via an API.
  - Required: retrieve all collected events for a given integrator.
  - Optional: filter by chain ID and contract address.
...
For reorganisation safety, the ingestor uses a simple confirmation-offset mechanism and only processes blocks that are a configured number of blocks behind the latest chain head.
...
- POLL_INTERVAL_MS (e.g. 5s)
- BLOCK_BATCH_SIZE (e.g. 2000)
- START_BLOCK=78600000

### API

The API component implements an endpoint for retrieving all events for a given integrator. The endpoint fetches the events directly from the MongoDB collection. Chain ID and contract address can be supplied as optional filters and otherwise default to the configured deployment values.
...
- Can improve the ingestion rate / horizontal scaling flexibility
- Robustness against single pod failures

Cons:

- More complex mechanism and operation (Kubernetes helps)
- Special care to rate limiting on RPCs

This was intentionally not selected for this assignment, as it is requires more work (code, tests, infra).

## Next Steps

- Implement a multiple worker approach
- Add integration tests with actual database
- Consider cursor-based pagination for `GET /events` if large offsets become inefficient.
- Add API metrics to monitor number of requests, failures, latency.
- Add ingestion metrics for block progress, ingestion lag, ingestion rate, RPC failures, and inserted event count.
- Further refine blockchain reorganisation safety mechanism
- Add rate limiting and authentication for API
